...
- [x] `src/pages/index.astro:117` — `{genre?.name} ({genre?.count})` has no null-guard, so the "Georgia" genre renders as "Georgia ()" in the genre `<select>`; guard with `?? 0` as already done in `genres.astro:40` (found: 2026-09-01) (resolved: 2026-09-22)
